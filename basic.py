...
class Parser:
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            module_def = res.register(self.module_def())
            if res.error: return res
            return res.success(module_def)
    ...
    def module_def(self):
        ...
        if self.current_tok.type == TT_ARROW:        
            ...
            return res.success(ModuleDefNode(
                var_name_tok,
                arg_name_toks,
                body,
                True
...
class RTResult:
    ...
    def reset(self):
        self.value = None
        self.error = None
        self.module_return_value = None
        self.loop_should_continue = False
        self.loop_should_break = False
    
    def register(self, res):
        if res.error: self.error = res.error
        self.module_return_value = res.module_return_value
    ...
    def success_return(self, value):
        self.reset()
        self.module_return_value = value
        return self
...
class Module(BaseModule):
    ...
    def execute(self, args):
        ...
        if res.should_return() and res.module_return_value == None: return res
        
        ret_value = (value if self.should_auto_return else None) or res.module_return_value or Number.null
        return res.success(ret_value)
    
    def copy(self):
        copy = Module(self.name, self.body_node, self.arg_names, self.should_auto_return)
        copy.set_context(self.context)
        copy.set_pos(self.pos_start, self.pos_end)
        return copy
...
class BuiltInModule(BaseModule):
    ...
    ###################################
    # Add built-in module here
    # name      : execute_<module_name>
    # example   : execute_print
    ###################################

    def execute_print(self, exec_ctx):
        print(str(exec_ctx.symbol_table.get('value')))
        return RTResult().success(Number.null)
    execute_print.arg_names = ['value'] # Argument for built-in module. In this case, execute_print will only have 1 argument called 'value'
    ...
    def execute_is_module(self, exec_ctx):
        is_module = isinstance(exec_ctx.symbol_table.get("value"), BaseModule)
        return RTResult().success(Number.true if is_module else Number.false)
...
BuiltInModule.is_module = BuiltInModule("is_module")
...
class Interpreter:
    ...
    ###################################
    # Add interpreter for node here
    # name      : visit_<node_name>
    # example   : visit_NumberNode
    ###################################
    ...
    def visit_ModuleDefNode(self, node, context):
        res = RTResult()

        module_name = node.var_name_tok.value if node.var_name_tok else None
        body_node = node.body_node
        arg_names = [arg_name.value for arg_name in node.arg_name_toks]
        module_value = Module(module_name, body_node, arg_names, node.should_auto_return).set_context(context).set_pos(node.pos_start, node.pos_end)
...
global_symbol_table.set("print", BuiltInModule.print)
global_symbol_table.set("print_ret", BuiltInModule.print_ret)
...
global_symbol_table.set("is_module", BuiltInModule.is_module)
